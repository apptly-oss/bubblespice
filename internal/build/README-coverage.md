# Coverage Testing
<!-- cspell:words carryforward coverpkg coverprofile gotest -->
...
- Runs `go test -coverprofile -coverpkg=./... ./...` for each module,
  using `go -C` to switch directories. This tests all packages
  recursively and instruments all packages for coverage, not just those
  with test files.
...
`make coverage` always writes HTML, function-level, and raw profile
reports to `.tmp/coverage/`. Open
`.tmp/coverage/coverage_<module>.html` in a browser to inspect
...
- **Proper attribution**: Code changes affect only the relevant module's
  coverage.
- **Carryforward-ready**: Per-module flags let Codecov's carryforward
  feature fill gaps when a module's coverage is absent from an upload,
  provided carryforward is enabled in the Codecov project settings.
